...
<style>
.main-container { width: 700px; max-width: 950px;}
</style>
...
* The **median** is the number in the middle of the data set. Half of the observations lie above the median and half below. The median is used as the measure of centrality when your data are not normally distributed and are instead skewed to the left of right.
 <center>
![](http://placekitten.com/200/200) ![](http://placekitten.com/200/200) ![](http://placekitten.com/200/200)
</center>
...
<p> This might look intimidating, but it means the same thing as \@ref(eq:equation1). Let's break the symbols apart a bit. The sigma means 'add up'. What are we adding up? All the heights "x", which we call our *observations*. The "i =" part indicates what number term to begin with. For our purposes, this will always be the first observation, when i = 1. The character on top of the sigma is the last observation we include in our summation. In this case it's n --because we're adding all n = 50 observations in each group of giraffes. In both equations, we still divide by the total number of observations in each group we have: n. </p>
<div style="margin-bottom:50px">
</div>
###Code it up
...
How reliably does the sample mean represent the population mean? Warning: when you calculate the mean from a small sample, you run the risk of calculating a sample mean that is very different from the true population mean! The larger of a sample you collect, the closer you will be to population mean, and the more reliable your sample mean will be.

Pressing 'Play' on the plot below will illustrate this concept. 
* Each point in this line is the mean from a random sample of a population with a TRUE mean of 0. [BAD SENTENCE!!] 
...
* The y-axis shows what the mean is for a sample of that particular size. Though the y-values vary here, remember that if the sample were a GOOD estimate of the population, the mean should be 0.  
* But you can see that when the samples are small the sample mean isn't always a good representation of the population it was sampled from--and that's not a good thing :-( 
