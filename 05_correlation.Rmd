...
:::note
**Note**: Here is an example note that we can use. It normally only works in bookdown sites, but since we're using `bookdown::html_document2`, we can use it here anytime we type `:::note` followed by our text on a new line, and then closed with another set of `:::` on a new line.  
:::
