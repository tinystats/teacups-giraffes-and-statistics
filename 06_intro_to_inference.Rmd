...
<center>
```{r fig.show="animate", animation.hook = 'gifski', fig.width=7, fig.height=3, echo=FALSE, message=FALSE, warning=FALSE, results = 'hide', interval=0.08, loop=FALSE}
library(ggplot2)
library(tweenr)
library(parallel)
library(MASS)
...
pf <- list(lapply(seq(1,53,2),function(x) ppl(x)), lapply(rep(53,3), function(x) ppl(x)), lapply(1:20, function(x) ppl2()))
return(pf)
}
mclapply(1:2, function(x) ppplot(x), mc.cores = 8, mc.cleanup = TRUE)
...
x <- round(mvrnorm(50, 9.8, 2.1, empirical = T))
m <- mean(x)
df <- data.frame(x = x, y = 23)
dfs <- list(df)
for(i in seq_len(nrow(df))) {
...
mclapply(rep(960, 20), function(x) plots4(x), mc.cores = 8, mc.cleanup = TRUE)
```
</center>
...
<center>
```{r, tut=FALSE, echo=FALSE, message= FALSE, warning=FALSE, fig.height=3.5, fig.width=6}
library(ggplot2)
