...
d <- lapply(1:4006, function(x) cor_var())
...
pd <- do.call(rbind, lapply(seq(39, 4000, 40), function(x) ff(x)))
...
p <- pd %>%
  ...
  layout( margin= m,
    ...
    yaxis = list(range=c(-0.1,4), zeroline=FALSE, title="Product of standard deviations")
    )%>%
  animation_slider(currentvalue = list(prefix = "Number of Samples: ", font = list(color="grey70", size=13)))

htmltools::save_html(p,"cov_vs_sxsy.html")

```
<center><iframe style="margin: 0;" src="cov_vs_sxsy.html" width="800" height="650" scrolling="yes" seamless="seamless" frameBorder="0"> </iframe></center>
...
<center>![](corr_curve.png){width=70%}</center>
