...
```
 ...
 (4) **Remove panel border** and (5) **Remove minor grid lines**
 
To make detailed changes to the layout, we can add a `theme()` argumemt. Nested within `theme()` we can use additional argumemts, such as `panel.border= ` and `panel.grid.minor=`. Many of `theme()`'s arguments can be set to `element_blank()` to remove the element in question. 

To read more about what can be modifed with `theme()`, check out this [resource](https://ggplot2.tidyverse.org/reference/theme.html).
...
Hopefully your PhD in the homeland will be satisfied with your new ggplot!

#Creates a Bell Curve
- TAke our histogram, overlay a bell curve.
- What does it mean? Density / probability in very loosely. 
